...
module.exports = {
  postVaidation: async (req, res, next) => {
    ...
    if (validateReq) {
      return res.status(400).json(failed(400, validateReq.message));
    }
    next();
  },
  ...
  putVaidation: async (req, res, next) => {
    ...
    if (errorField === null) {
      return res
        .status(400)
        .json(failed(400, "error, check correct response "));
    }
    next();
  },
  ...
  patchVaidation: async (req, res, next) => {
    ...
    if (errorField === null) {
      return res.status(400).json(failed(400, "favorite field not correct"));
    }
    next();
  },
};
